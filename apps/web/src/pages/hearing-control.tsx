...
const CONFIRM_ACTIONS: Record<string, { label: string; desc: string; variant: 'destructive' | 'warning' }> = {
  end:     { label: 'Tutup Sidang',   desc: 'Sidang akan ditutup secara permanen. Tindakan ini tidak dapat dibatalkan. Pastikan semua pemeriksaan sudah selesai.',          variant: 'destructive' },
  suspend: { label: 'Skors Sidang',   desc: 'Sidang akan diskors sementara. Seluruh peserta akan menunggu di ruang tunggu sampai sidang dilanjutkan kembali.',              variant: 'warning'     },
};

const STATE_VARIANT: Record<string, 'success' | 'warning' | 'destructive' | 'outline'> = {
  STARTED: 'success', READY: 'success',
  SUSPENDED: 'warning', NOT_READY: 'outline',
  ENDED: 'outline', POSTPONED: 'destructive',
};

const EVENT_LABEL: Record<string, string> = {
  HEARING_STARTED: 'Sidang dibuka', HEARING_SUSPENDED: 'Sidang diskors',
  HEARING_RESUMED: 'Sidang dilanjutkan', HEARING_ENDED: 'Sidang ditutup',
...
export function HearingControlPage() {
  ...
  async function directAction(name: string) { await doAction(name); }

  // Aksi dengan konfirmasi
  function requestConfirm(name: keyof typeof CONFIRM_ACTIONS) { setPending(name); }
  async function confirmAction() { if (pending) await doAction(pending); }
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Gunakan persona Hakim untuk mengoperasikan kontrol sidang.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-5">
          ...
          <div className="rounded-lg bg-slate-50 p-3 text-xs text-slate-500">
            {state === 'NOT_READY' && '⚠ Semua gate belum terpenuhi. Selesaikan checklist kesiapan dan provisioning ruang virtual terlebih dahulu.'}
            {state === 'READY'     && '✅ Sidang siap dibuka. Hakim dapat membuka sidang.'}
            {state === 'STARTED'   && '🟢 Sidang berlangsung. Hakim dapat menskors atau menutup sidang.'}
            {state === 'SUSPENDED' && '⏸ Sidang sedang diskors. Hakim dapat melanjutkan atau menutup.'}
      ...
      {/* ── Panel timeline ── */}
      <Card>
        <CardHeader><CardTitle>Timeline Kejadian</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {events.length === 0 && (
            <p className="py-6 text-center text-sm text-slate-400">Belum ada kejadian tercatat untuk sidang ini.</p>
          )}
          {(Array.isArray(events) ? events : []).map((event: RuntimeEvent) => (
            <div key={event.id} className="rounded-lg border p-3">
              <div className="font-semibold text-sm">{EVENT_LABEL[event.eventType] ?? event.eventType}</div>
              {event.reason && <div className="mt-0.5 text-xs text-slate-500">{event.reason}</div>}
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <AlertTriangle className={`h-5 w-5 ${confirmData?.variant === 'destructive' ? 'text-red-500' : 'text-amber-500'}`} />
