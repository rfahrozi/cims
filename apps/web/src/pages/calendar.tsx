      ...
      <CardContent className="pt-6">
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="space-y-1.5 flex-1 min-w-[200px]">
            ...
            <Input value={orgId} onChange={e => setOrgId(e.target.value)} placeholder="Misal: court-demo" className="h-9" />
          </div>
            ...
            <div key={date}>
              ...
              <div className="grid gap-3 lg:grid-cols-2">
                {groupedEvents[date].map(evt => (
                  <div key={evt.id} className="rounded-xl border bg-slate-50 p-4 transition-colors hover:bg-white hover:shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                      <div className="font-semibold text-blue-900">{evt.case_number}</div>
                      <Badge variant="outline" className="bg-white">{evt.hearing_type}</Badge>
                    </div>
                    {evt.case_title && <div className="text-xs text-slate-600 mb-2">{evt.case_title}</div>}
                    ...
                    <div className="mt-3 text-sm grid grid-cols-2 gap-1 bg-white p-2 border rounded-md">
                      ...
                      <div className="font-medium text-xs">{new Date(evt.start_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</div>
                      <div className="text-slate-500 text-xs">Selesai:</div>
                      <div className="font-medium text-xs">{new Date(evt.end_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</div>
                    </div>

                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {evt.resources.map((r, i) => (
                        <span key={i} className="rounded-full bg-blue-100 text-blue-700 px-2 py-0.5 text-[10px] font-medium border border-blue-200">
