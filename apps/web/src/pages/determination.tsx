...
type DeterminationResult = {
  id: string; version: number; decision: string; hearing_mode: string;
  official_reference: string; reason: string; created_at: string;
};
...
export function DeterminationPage() {
  ...
  const [reason, setReason] = useState('Sesuai dengan permohonan dan kelayakan teknis, sidang dilaksanakan secara elektronik.');
    ...
    <div className="grid gap-5 lg:grid-cols-[1.2fr_1fr]">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Hanya persona Hakim yang dapat menyimpan penetapan ini.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input value={reference} onChange={(e: ChangeEvent<HTMLInputElement>) => setReference(e.target.value)} />
          </div>
          <div className="space-y-2">
          ...
          <Card className="border-green-200 bg-green-50">
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2 text-green-800">
                ...
                <CardTitle className="text-base text-green-800">Penetapan Berhasil Disimpan</CardTitle>
              </div>
            </CardHeader>
            <CardContent className="space-y-2 text-sm text-green-900">
              ...
              <div className="mt-3 rounded bg-green-100 p-2 text-xs">
                {result.reason}
              </div>
            </CardContent>
          </Card>
        ...
        {!result && !error && (
          <Card className="border-dashed">
            <CardContent className="pt-6 text-center">
              ...
              <p className="text-sm text-slate-400">Silakan isi dan simpan form penetapan di sebelah kiri.</p>
            </CardContent>
          </Card>
        )}
      </div>
