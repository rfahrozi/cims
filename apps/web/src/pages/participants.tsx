...
const ROLE_LABEL: Record<string, string> = {
  ...
  JUDGE: 'Hakim',
  COURT_CLERK: 'Panitera',
  CORRECTIONS_OFFICER: 'Petugas Pemasyarakatan',
  IT_OPERATOR: 'Operator TI'
};

const STATE_VARIANT: Record<string, 'success' | 'warning' | 'outline'> = {
  ADMITTED: 'success',
  WAITING: 'warning',
  TOKEN_ISSUED: 'warning',
  REGISTERED: 'outline',
  LEFT: 'outline',
  REMOVED: 'outline',
  REVOKED: 'outline'
...
export function ParticipantsPage() {
  ...
  async function register() {
    ...
    try {
      ...
      setSuccess(
        `Peserta "${displayName.trim()}" berhasil didaftarkan sebagai ${ROLE_LABEL[role] ?? role}.`
      );
      setDisplayName('');
      setAlias('');
      await refresh();
  ...
  async function issue(id: string) {
    ...
    try {
      ...
      setToken(data.token);
      setSuccess(
        'Token akses berhasil dibuat. Token hanya ditampilkan sekali — simpan dan kirimkan ke peserta.'
      );
      await refresh();
    } catch (e) {
      setError(e);
  ...
  async function admit(id: string) {
    ...
    try {
      await api(`/hearings/${hearingId}/participants/${id}/admit`, {
        method: 'POST',
        body: JSON.stringify({ roomCode: 'MAIN' })
      });
  ...
  return (
    <>
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_1.2fr]">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Daftarkan peserta</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
              ...
              {protectedIdentity && (
                <div className="space-y-1.5 pl-5">
                  ...
                  <p className="text-[11px] text-slate-500 leading-tight">
                    Nama asli akan disembunyikan dari peserta lain, kecuali Hakim dan Panitera.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3 rounded-lg border border-blue-100 bg-blue-50/50 p-3">
              ...
              {role === 'ADVOCATE' &&
                locationType !== 'COURT' &&
                locationType !== 'CORRECTIONS' && (
                  <div className="space-y-1.5 pt-1">
                    <Label className="text-xs text-amber-700">
                      Referensi Penetapan Pisah Lokasi (Wajib)
                    </Label>
                    <Input
            ...
            <Button onClick={register} disabled={!displayName.trim()}>
              Daftarkan Peserta
            </Button>
            ...
            {token && (
              <div className="rounded-lg border border-amber-300 bg-amber-50 p-3">
                <div className="mb-1 text-xs font-semibold text-amber-800">
                  ⚠️ Token akses — tampil sekali saja
                </div>
                <code className="block break-all text-xs text-amber-900">{token}</code>
                <p className="mt-2 text-xs text-amber-700">
                  Salin dan kirimkan token ini ke peserta. Token berlaku 15 menit.
                </p>
              </div>
            )}
          </CardContent>
          ...
          <CardContent className="space-y-3">
            ...
            {!query.isLoading && participants.length === 0 && (
              <EmptyState
                ...
                action={{
                  label: 'Isi nama dan daftarkan',
                  onClick: () => document.getElementById('participant-name')?.focus()
                }}
              />
            )}
            ...
            {participants.map((p) => (
              <div key={p.id} className="rounded-xl border bg-white p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <Button size="sm" variant="outline" onClick={() => issue(p.id)}>
                    Buat Token Akses
                  </Button>
                  {p.state === 'WAITING' && (
                    <Button size="sm" onClick={() => admit(p.id)}>
                      Izinkan Masuk
                    </Button>
                  )}
                </div>
