...
export function IncidentsPage() {
  ...
  const [description, setDescription] = useState(
    'Audio dari lokasi peserta tidak terdengar dengan jelas.'
  );
  ...
  return (
    <>
      ...
      <div className="grid gap-5 xl:grid-cols-[.9fr_1.3fr]">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              {type === 'CYBER' && (
                <p className="text-xs text-amber-600">
                  ⏱ Wajib notifikasi dalam 1×24 jam setelah kejadian.
                </p>
              )}
              {type === 'FORCE_MAJEURE' && (
                <p className="text-xs text-amber-600">
                  ⏱ Wajib notifikasi dalam 3×24 jam setelah kejadian.
                </p>
              )}
            </div>
            <div className="space-y-2">
              ...
              {(severity === 'CRITICAL' || severity === 'HIGH') && (
                <p className="text-xs text-red-600">
                  ⚠ Insiden {SEVERITY_LABEL[severity]} akan otomatis menskors sidang yang sedang
                  berlangsung.
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Jelaskan apa yang terjadi, dampak, dan tindakan awal yang sudah diambil."
              />
            </div>
            <Button onClick={create} disabled={!title || !description}>
              Simpan Insiden
            </Button>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Timeline Insiden</CardTitle>
          </CardHeader>
              ...
              <div key={i.id} className="rounded-xl border p-4">
                ...
                {i.notificationDeadline && !i.notifiedAt && (
                  <div className="mt-2 rounded bg-red-50 px-2 py-1 text-xs text-red-700">
                    ⏰ Batas pemberitahuan:{' '}
                    {new Date(i.notificationDeadline).toLocaleString('id-ID')}
                  </div>
                )}
                {i.notifiedAt && (
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => action(i.id, 'START_MITIGATION')}
                  >
                    Mulai Mitigasi
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => action(i.id, 'RESOLVE')}>
                    Selesaikan
                  </Button>
                  <Button size="sm" onClick={() => action(i.id, 'CLOSE')}>
                    Tutup
                  </Button>
                </div>
              </div>
