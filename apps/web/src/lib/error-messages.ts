...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  SCHEDULE_REQUIRED:
    'Jadwal sidang harus disetujui terlebih dahulu sebelum melanjutkan.',
  NOTICE_ACK_REQUIRED:
  ...
  UNAUTHORIZED:
    'Sesi Anda telah habis. Silakan masuk kembali.',
  ...
  DELEGATION_SELF_FORBIDDEN:
    'Tidak diperbolehkan mendelegasikan kewenangan kepada diri sendiri.',
  ...
  INVALID_HEARING_TRANSITION:
    'Aksi tidak dapat dilakukan pada status sidang saat ini.',

  // ── Peserta & sesi ───────────────────────────────────────────────────────
  INVALID_PARTICIPANT_TRANSITION:
    'Perubahan status peserta tidak diizinkan dari status saat ini.',
  JOIN_TOKEN_EXPIRED:
    'Tautan akses sidang telah kedaluwarsa. Minta Panitera untuk menerbitkan tautan baru.',
  JOIN_TOKEN_REVOKED:
    'Tautan akses sidang telah dicabut. Hubungi Panitera.',
  JOIN_TOKEN_INVALID:
    'Tautan akses sidang tidak valid.',
  JOIN_TOKEN_CONSUMED:
    'Tautan akses sidang ini sudah pernah digunakan.',
  ...
  INVALID_INCIDENT_TRANSITION:
    'Status insiden tidak dapat diubah dengan aksi tersebut.',
  INVALID_INCIDENT_TIME:
    'Waktu kejadian insiden tidak valid. Periksa format tanggal dan waktu.',

  // ── Putusan banding ──────────────────────────────────────────────────────
  APPEAL_READING_NOT_FOUND:
    'Data pembacaan putusan banding tidak ditemukan.',
  ...
  APPEAL_EXCERPT_ALREADY_PUBLISHED:
    'Petikan putusan sudah pernah diunggah untuk pembacaan ini.',
  APPEAL_ALREADY_TRANSMITTED:
    'Berkas sudah pernah dikirim ke pengadilan tingkat pertama.',
  APPEAL_NOT_READ_YET:
    'Berkas hanya dapat dikirim setelah pembacaan putusan selesai (status READ).',

  // ── Mutasi tahanan ───────────────────────────────────────────────────────
  CUSTODY_TRANSFER_NOT_FOUND:
    'Data mutasi tahanan tidak ditemukan.',
  CUSTODY_NOTIF_NOT_FOUND:
    'Notifikasi mutasi tidak ditemukan.',

  // ── Liaison ──────────────────────────────────────────────────────────────
  LIAISON_NOT_FOUND:
    'Data pejabat penghubung tidak ditemukan.',
  DELEGATION_NOT_FOUND:
    'Data delegasi tidak ditemukan.',
  ESCALATION_NOT_FOUND:
    'Data eskalasi tidak ditemukan.',

  // ── Data & validasi ──────────────────────────────────────────────────────
  VALIDATION_ERROR:
    'Data yang dimasukkan tidak valid. Periksa kembali semua field yang diperlukan.',
  HEARING_NOT_FOUND:
    'Data persidangan tidak ditemukan. Pastikan perkara yang dipilih sudah benar.',
  ...
  // ── Governance ───────────────────────────────────────────────────────────
  INVALID_CLOSED_AT:
    'Tanggal penutupan perkara tidak valid.',
  ACCESS_REVIEW_SCOPE_REQUIRED:
    'Cakupan tinjauan akses harus mencakup hearing atau organisasi.',
};
...
export function errorMessage(error: unknown): string {
  ...
  if (raw.includes('HTTP 422')) return 'Data yang dikirim tidak dapat diproses. Periksa kembali isian form.';
  if (raw.includes('HTTP 429')) return 'Terlalu banyak permintaan. Tunggu beberapa saat lalu coba kembali.';
