...
export function PersonaSwitcher() {
  ...
  useEffect(() => {
    const listener = () => setValue(getPersona());
    window.addEventListener('cims-persona-change', listener);
    return () => window.removeEventListener('cims-persona-change', listener);
  }, []);
  return (
    <div className="space-y-2 rounded-lg border border-white/15 bg-white/5 p-3">
      ...
      <Select
        ...
        onValueChange={(next: string) => {
          setPersona(next as Persona);
          setValue(next as Persona);
        }}
      >
        <SelectTrigger className="border-white/20 bg-white text-slate-900">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
