    ...
    <div className="space-y-2 rounded-lg border border-white/15 bg-white/5 p-3">
      ...
      >
        <SelectTrigger className="border-white/20 bg-white text-slate-900">
          <SelectValue placeholder="Pilih persidangan" />
        </SelectTrigger>
        <SelectContent>
          {(hearings || []).map((item) => (
            <SelectItem key={item.id} value={item.id}>
              {item.caseNumber} · #{item.hearingSequence ?? 1}
            </SelectItem>
          ))}
        </SelectContent>
      </Select>
      <p className="line-clamp-2 text-[11px] leading-4 text-blue-200">
        Pilihan ini dipakai oleh seluruh modul workflow.
      </p>
    </div>
  );
}
