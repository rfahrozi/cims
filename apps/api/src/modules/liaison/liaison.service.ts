...
export class LiaisonService {
  ...
  async createUnit(
    ...
    if (!this.mode.postgres) {
      const item: OrgUnit = {
        ...
        jurisdictionArea: dto.jurisdiction_area,
        active: true,
        createdAt: now
      };
  ...
  async listUnits(organizationId: string): Promise<OrgUnit[]> {
    if (!this.mode.postgres)
      return this.mem().units.filter((u) => u.organizationId === organizationId && u.active);
    return this.pg.transactionAs(this.systemUser(), async (c) => {
      const rows = (
        await c.query(
          `select id,organization_id,unit_code,unit_name,unit_type,jurisdiction_area,active,created_at::text from organization_units where organization_id=$1 and active=true order by unit_name`,
          [organizationId]
        )
      ).rows;
  ...
  async createLiaison(
    user: CurrentUser,
    dto: CreateLiaisonOfficerDto,
    correlationId?: string
  ): Promise<LiaisonOfficer> {
    requireRoles(user, ['SYSTEM_ADMIN', 'COURT_CLERK']);
    const now = new Date().toISOString();
    if (!this.mode.postgres) {
      const item: LiaisonOfficer = {
        id: this.memory.id(),
        userId: dto.user_id,
        userName: dto.user_name,
        organizationId: dto.organization_id,
        organizationUnitId: dto.organization_unit_id,
        appointedFrom: dto.appointed_from,
        appointedUntil: dto.appointed_until,
        appointmentReference: dto.appointment_reference,
        active: true,
      ...
      await this.audit.record(
        'LIAISON_OFFICER_APPOINTED',
        'ORGANIZATION',
        dto.organization_id,
        user,
        { user_id: dto.user_id, appointment_reference: dto.appointment_reference },
        correlationId
      );
      return item;
    }
        ...
        await c.query(
         ...
         returning id,user_id,user_name,organization_id,organization_unit_id,appointed_from::text,appointed_until::text,appointment_reference,active,contact_email,contact_phone,appointed_by,created_at::text,row_version`,
          [
            ...
            dto.user_name,
            dto.organization_id,
            dto.organization_unit_id ?? null,
            dto.appointed_from,
            dto.appointed_until ?? null,
            dto.appointment_reference,
  ...
  async listLiaisons(organizationId: string, user: CurrentUser): Promise<LiaisonOfficer[]> {
    ...
    if (!this.mode.postgres)
      return this.mem().liaisons.filter((l) => l.organizationId === organizationId && l.active);
    return this.pg.transactionAs(user, async (c) => {
      const rows = (
        await c.query(
          `select id,user_id,user_name,organization_id,organization_unit_id,appointed_from::text,appointed_until::text,appointment_reference,active,contact_email,contact_phone,appointed_by,created_at::text,row_version from liaison_officers where organization_id=$1 and active=true order by appointed_from desc`,
          [organizationId]
        )
      ).rows;
  ...
  async deactivateLiaison(
    ...
    if (!this.mode.postgres) {
      ...
      l.rowVersion++;
      await this.audit.record(
        'LIAISON_OFFICER_DEACTIVATED',
        'ORGANIZATION',
        l.organizationId,
        user,
        { liaison_id: id, reason },
        correlationId
      );
      return l;
    }
    return this.pg.transactionAs(user, async (c) => {
      const row = (
        await c.query(
          `update liaison_officers set active=false,row_version=row_version+1,updated_at=now() where id=$1 returning id,user_id,user_name,organization_id,organization_unit_id,appointed_from::text,appointed_until::text,appointment_reference,active,contact_email,contact_phone,appointed_by,created_at::text,row_version`,
          [id]
        )
      ).rows[0];
  ...
  async createDelegation(
    ...
    if (dto.delegate_user_id === user.id)
      throw new DomainError('DELEGATION_SELF_FORBIDDEN', 'Cannot delegate to yourself.', 409);
    const now = new Date().toISOString();
    if (!this.mode.postgres) {
      const item: Delegation = {
        ...
        delegateUserId: dto.delegate_user_id,
        delegateName: dto.delegate_name,
        organizationId: dto.organization_id,
        scope: dto.scope,
        validFrom: dto.valid_from,
        validUntil: dto.valid_until,
        delegationReason: dto.delegation_reason,
      ...
      await this.audit.record(
        ...
        dto.organization_id,
        user,
        { delegate_user_id: dto.delegate_user_id, scope: dto.scope },
        correlationId
      );
      return item;
    }
  ...
  async listDelegations(organizationId: string, user: CurrentUser): Promise<Delegation[]> {
    ...
    if (!this.mode.postgres)
      return this.mem().delegations.filter(
        (d) => d.organizationId === organizationId && d.status === 'ACTIVE'
      );
    return this.pg.transactionAs(user, async (c) => {
      const rows = (
        await c.query(
          `select id,delegator_user_id,delegator_name,delegate_user_id,delegate_name,organization_id,scope,valid_from::text,valid_until::text,delegation_reason,official_reference,status,created_by,created_at::text from delegations where organization_id=$1 and status='ACTIVE' order by valid_from desc`,
          [organizationId]
        )
      ).rows;
  ...
  async revokeDelegation(
    ...
    if (!this.mode.postgres) {
      ...
      await this.audit.record(
        ...
        d.organizationId,
        user,
        { delegation_id: id, reason: dto.reason },
        correlationId
      );
      return d;
    }
    return this.pg.transactionAs(user, async (c) => {
      const row = (
        await c.query(
          `update delegations set status='REVOKED',revoked_by=$2,revoked_at=now() where id=$1 returning id,delegator_user_id,delegator_name,delegate_user_id,delegate_name,organization_id,scope,valid_from::text,valid_until::text,delegation_reason,official_reference,status,created_by,created_at::text,revoked_by,revoked_at::text`,
          [id, user.id]
        )
      ).rows[0];
  ...
  async createEscalation(
    ...
    if (!this.mode.postgres) {
      const item: Escalation = {
        ...
        liaisonOfficerId: dto.liaison_officer_id,
        escalationType: dto.escalation_type,
        description: dto.description,
        escalatedTo: dto.escalated_to,
        escalatedToName: dto.escalated_to_name,
        status: 'OPEN',
        createdAt: now,
        updatedAt: now
      };
      ...
      await this.audit.record(
        'ESCALATION_CREATED',
        dto.hearing_id ? 'HEARING' : 'ORGANIZATION',
        dto.hearing_id ?? dto.liaison_officer_id,
        user,
        { escalation_type: dto.escalation_type },
        correlationId
      );
      return item;
    }
      ...
      const row = (
        await c.query(
         ...
         returning id,hearing_id,liaison_officer_id,escalation_type,description,escalated_to,escalated_to_name,status,resolution_notes,resolved_at::text,created_at::text,updated_at::text`,
          [
            dto.hearing_id ?? null,
            dto.liaison_officer_id,
            dto.escalation_type,
            dto.description,
            dto.escalated_to,
            dto.escalated_to_name
          ]
        )
      ).rows[0];
      await this.audit.record(
        'ESCALATION_CREATED',
        dto.hearing_id ? 'HEARING' : 'ESCALATION',
        dto.hearing_id ?? row.id,
        user,
        { escalation_type: dto.escalation_type },
        correlationId
      );
  ...
  async listEscalations(
    ...
    liaisonOfficerId: string | undefined,
    user: CurrentUser
  ): Promise<Escalation[]> {
    requireRoles(user, ['LIAISON_OFFICER', 'COURT_CLERK', 'SYSTEM_ADMIN', 'AUDITOR']);
    if (!this.mode.postgres) {
      return this.mem().escalations.filter(
        (e) =>
  ...
  async resolveEscalation(
    ...
    if (!this.mode.postgres) {
      ...
      e.status = 'RESOLVED';
      e.resolutionNotes = dto.resolution_notes;
      e.resolvedAt = now;
      e.updatedAt = now;
      await this.audit.record(
        'ESCALATION_RESOLVED',
        e.hearingId ? 'HEARING' : 'ESCALATION',
        e.hearingId ?? id,
        user,
    ...
    return this.pg.transactionAs(user, async (c) => {
      const row = (
        await c.query(
          `update liaison_escalations set status='RESOLVED',resolution_notes=$2,resolved_at=now(),updated_at=now() where id=$1 returning id,hearing_id,liaison_officer_id,escalation_type,description,escalated_to,escalated_to_name,status,resolution_notes,resolved_at::text,created_at::text,updated_at::text`,
          [id, dto.resolution_notes]
        )
      ).rows[0];
      if (!row) throw new DomainError('ESCALATION_NOT_FOUND', 'Escalation not found.', 404);
      await this.audit.record(
        'ESCALATION_RESOLVED',
        row.hearing_id ? 'HEARING' : 'ESCALATION',
        row.hearing_id ?? id,
        user,
        { resolution_notes: dto.resolution_notes },
        correlationId
  ...
  private mem() {
    const s = this.memory as unknown as {
      liaisonUnits?: OrgUnit[];
      liaisonOfficers?: LiaisonOfficer[];
      liaisonDelegations?: Delegation[];
      liaisonEscalations?: Escalation[];
    };
    s.liaisonUnits ??= [];
    s.liaisonOfficers ??= [];
    s.liaisonDelegations ??= [];
    s.liaisonEscalations ??= [];
    return {
      units: s.liaisonUnits,
      liaisons: s.liaisonOfficers,
      delegations: s.liaisonDelegations,
      escalations: s.liaisonEscalations
    };
  }
  ...
  private mapUnit(r: Record<string, unknown>): OrgUnit {
    return {
      ...
      jurisdictionArea: r.jurisdiction_area ? String(r.jurisdiction_area) : undefined,
  ...
  private mapLiaison(r: Record<string, unknown>): LiaisonOfficer {
    return {
      ...
      organizationId: String(r.organization_id),
      organizationUnitId: r.organization_unit_id ? String(r.organization_unit_id) : undefined,
      appointedFrom: String(r.appointed_from),
      appointedUntil: r.appointed_until ? String(r.appointed_until) : undefined,
      appointmentReference: String(r.appointment_reference),
      active: Boolean(r.active),
      ...
      appointedBy: String(r.appointed_by),
      createdAt: String(r.created_at),
  ...
  private mapEscalation(r: Record<string, unknown>): Escalation {
    return {
      id: String(r.id),
      hearingId: r.hearing_id ? String(r.hearing_id) : undefined,
      liaisonOfficerId: String(r.liaison_officer_id),
      escalationType: String(r.escalation_type),
      description: String(r.description),
      escalatedTo: String(r.escalated_to),
      escalatedToName: String(r.escalated_to_name),
      status: String(r.status),
