...
export class ProviderWebhookService {
  ...
  async ingest(
    ...
    if (!this.secret || this.secret.length < 16) throw new DomainError('WEBHOOK_CONFIG_INVALID', 'Webhook shared secret is not configured.', 503);
    const timestamp = headers.timestamp;
    const signature = headers.signature;
    if (!timestamp || !signature) throw new DomainError('WEBHOOK_SIGNATURE_REQUIRED', 'Webhook signature and timestamp are required.', 401);
    const seconds = Number(timestamp);
    if (!Number.isFinite(seconds) || Math.abs(Date.now() / 1000 - seconds) > this.toleranceSeconds) {
      throw new DomainError('WEBHOOK_TIMESTAMP_INVALID', 'Webhook timestamp is outside the accepted tolerance.', 401);
    }
    const expected = `sha256=${createHmac('sha256', this.secret).update(`${timestamp}.`).update(rawBody).digest('hex')}`;
    const valid = signature.length === expected.length && timingSafeEqual(Buffer.from(signature), Buffer.from(expected));
    if (!valid) throw new DomainError('WEBHOOK_SIGNATURE_INVALID', 'Webhook signature is invalid.', 401);
    ...
    if (!this.mode.postgres) {
      ...
      this.metrics.increment('provider_webhook_events_total', { provider: providerCode, event_type: eventType, status: 'PROCESSED' });
      return { status: 'PROCESSED', event_id: eventId, mode: 'MEMORY' };
    }

    const inserted = await this.pg.transaction(async (client) => {
      await client.query("select set_config('cims.is_system_admin','true',true),set_config('cims.hearing_assignments','',true),set_config('cims.organization_ids','',true)");
    ...
    if (payload.hearing_id) {
      await this.audit.append({
        ...
        payload: { provider: providerCode, provider_event_id: eventId, provider_event_type: eventType },
      });
    }
    return { status: 'PROCESSED', event_id: eventId };
  }

  private async applyEvent(client: import('pg').PoolClient, payload: ProviderWebhookPayload, eventType: string, eventId: string): Promise<void> {
    ...
    if (participantId && hearingId && ['participant.waiting', 'participant.admitted', 'participant.left'].includes(eventType)) {
      const eventMap = {
        'participant.waiting': { attendance: 'WAITING_JOINED', state: 'WAITING', session: 'WAITING', room: 'WAITING' },
        'participant.admitted': { attendance: 'ADMITTED', state: 'ADMITTED', session: 'ADMITTED', room: roomCode ?? 'MAIN' },
        'participant.left': { attendance: 'LEFT', state: 'LEFT', session: 'LEFT', room: roomCode ?? null },
      } as const;
      ...
      await client.query(
        ...
        [hearingId, participantId, mapped.attendance, mapped.room, eventId, occurredAt, createHash('sha256').update(JSON.stringify(payload)).digest('hex')],
      );
      await client.query(
    ...
    if (eventType === 'recording.completed' && String(payload.room_code ?? '') === 'CONSULTATION') {
      await client.query(
        ...
        [hearingId ?? null, JSON.stringify({ provider_event_id: eventId, provider_session_reference: payload.provider_session_reference ?? null })],
      );
    }
  }
}
