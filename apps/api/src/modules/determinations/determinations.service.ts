...
export class DeterminationsService {
  constructor(private readonly core: CoreWorkflowRepository, private readonly audit: AuditService) {}
  ...
  async createDetermination(user: CurrentUser, dto: CreateDeterminationDto, correlationId?: string) {
  ...
  private publicDetermination(item: Awaited<ReturnType<CoreWorkflowRepository['createDetermination']>>) {
    return {
      id: item.id,
      hearing_id: item.hearingId,
      version: item.version,
