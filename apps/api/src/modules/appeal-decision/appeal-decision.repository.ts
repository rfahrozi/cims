...
export class AppealDecisionRepository {
  ...
  async create(input: {
    ...
    createdBy: string;
  }, user: CurrentUser): Promise<AppealDecisionReading> {
    if (!this.mode.postgres) {
      ...
      store.readings.forEach(r => { if (r.hearingId === input.hearingId && r.status === 'SCHEDULED') r.status = 'SUPERSEDED'; });
    ...
    return this.pg.transactionAs(user, async (client) => {
      ...
      const row = (await client.query(
        ...
        [input.hearingId, version, input.scheduledAt, input.displayTimezone, input.deliveryMode,
          input.determinationReference, input.virtualSessionReference ?? null, input.openToPublic, input.createdBy],
      )).rows[0];
  ...
  async reschedule(id: string, input: {
    ...
    if (!this.mode.postgres) {
      ...
      if (r.status !== 'SCHEDULED') throw new DomainError('APPEAL_READING_NOT_SCHEDULED', 'Only SCHEDULED readings can be rescheduled.', 409);
      Object.assign(r, {
      ...
      const row = (await client.query(
        `update appeal_decision_readings
         ...
         returning id,hearing_id,version,scheduled_at::text,display_timezone,delivery_mode,
                   ...
                   read_at::text,cassation_deadline_at::text,cassation_deadline_note,
                   reschedule_reason,created_by,created_at::text,updated_at::text,row_version`,
        [id, input.scheduledAt, input.deliveryMode, input.rescheduleReason,
          input.determinationReference ?? null, input.virtualSessionReference ?? null],
      )).rows[0];
      if (!row) throw new DomainError('APPEAL_READING_NOT_FOUND', 'Appeal reading not found or not SCHEDULED.', 404);
      return this.mapReading(row);
    });
  }

  async markRead(id: string, readAt: string, cassationDeadlineAt: string | undefined, user: CurrentUser): Promise<AppealDecisionReading> {
    if (!this.mode.postgres) {
      const store = this.memStore();
      const r = store.readings.find(x => x.id === id);
      if (!r) throw new DomainError('APPEAL_READING_NOT_FOUND', 'Appeal reading not found.', 404);
      r.status = 'READ'; r.readAt = readAt;
      if (cassationDeadlineAt) r.cassationDeadlineAt = cassationDeadlineAt;
      r.updatedAt = new Date().toISOString(); r.rowVersion++;
      return r;
    }
    return this.pg.transactionAs(user, async (client) => {
      const row = (await client.query(
  ...
  async list(hearingId: string, user: CurrentUser): Promise<AppealDecisionReading[]> {
    if (!this.mode.postgres) return this.memStore().readings.filter(r => r.hearingId === hearingId);
    return this.pg.transactionAs(user, async (client) => {
  ...
  async createNoticeStep(input: {
    ...
    createdBy: string;
  }, user: CurrentUser): Promise<AppealNoticeStep> {
    if (!this.mode.postgres) {
      const item: AppealNoticeStep = {
        id: this.memory.id(), status: 'PENDING',
    ...
    return this.pg.transactionAs(user, async (client) => {
      const row = (await client.query(
        ...
        [input.readingId, input.stepCode, input.senderOrganizationId, input.recipientReference,
          input.recipientName, input.channel, input.officialReference, input.createdBy],
      )).rows[0];
      return this.mapNoticeStep(row);
    });
  }

  async updateNoticeStep(id: string, status: AppealNoticeStepStatus, receiptReference: string | undefined, user: CurrentUser): Promise<AppealNoticeStep> {
    ...
    if (!this.mode.postgres) {
      ...
      if (status === 'ACKNOWLEDGED') { s.acknowledgedAt = now; s.receiptReference = receiptReference; }
      return s;
    }
    return this.pg.transactionAs(user, async (client) => {
      const row = (await client.query(
      ...
      if (!row) throw new DomainError('APPEAL_NOTICE_STEP_NOT_FOUND', 'Notice step not found.', 404);
      return this.mapNoticeStep(row);
    });
  }

  async listNoticeSteps(readingId: string, user: CurrentUser): Promise<AppealNoticeStep[]> {
    if (!this.mode.postgres) return this.memStore().noticeSteps.filter(s => s.readingId === readingId);
    return this.pg.transactionAs(user, async (client) => {
      const rows = (await client.query(
  ...
  async recordPresence(input: {
    ...
    notes?: string;
    verifiedBy: string;
  }, user: CurrentUser): Promise<AppealPresenceRecord> {
    const now = new Date().toISOString();
    if (!this.mode.postgres) {
      const existing = this.memStore().presenceRecords.findIndex(
        p => p.readingId === input.readingId && p.partyRole === input.partyRole && p.partyReference === input.partyReference,
      );
    ...
    return this.pg.transactionAs(user, async (client) => {
      const row = (await client.query(
        ...
        [input.readingId, input.partyRole, input.partyReference, input.partyName,
          input.attendanceStatus, input.attendanceMode, input.notes ?? null, input.verifiedBy, now],
      )).rows[0];
  ...
  async listPresence(readingId: string, user: CurrentUser): Promise<AppealPresenceRecord[]> {
    if (!this.mode.postgres) return this.memStore().presenceRecords.filter(p => p.readingId === readingId);
    return this.pg.transactionAs(user, async (client) => {
      const rows = (await client.query(
  ...
  async publishExcerpt(input: {
    ...
    publishedBy: string;
    notes?: string;
  }, user: CurrentUser): Promise<AppealPublication> {
    if (!this.mode.postgres) {
      const existing = this.memStore().publications.find(p => p.readingId === input.readingId);
      if (existing) throw new DomainError('APPEAL_EXCERPT_ALREADY_PUBLISHED', 'Petikan putusan sudah dipublikasikan.', 409);
      const item: AppealPublication = { id: this.memory.id(), createdAt: new Date().toISOString(), ...input };
    ...
    return this.pg.transactionAs(user, async (client) => {
      const row = (await client.query(
        ...
        [input.readingId, input.excerptReference, input.sourceSystemCode, input.documentHash ?? null,
          input.publishedAt, input.sameDayCompliant, input.publishedBy, input.notes ?? null],
      )).rows[0];
  ...
  async transmit(input: {
    ...
    notes?: string;
  }, user: CurrentUser): Promise<AppealTransmission> {
    if (!this.mode.postgres) {
      const existing = this.memStore().transmissions.find(t => t.readingId === input.readingId);
      if (existing) throw new DomainError('APPEAL_ALREADY_TRANSMITTED', 'Berkas sudah dikirim ke pengadilan tingkat pertama.', 409);
      const item: AppealTransmission = { id: this.memory.id(), createdAt: new Date().toISOString(), ...input };
    ...
    return this.pg.transactionAs(user, async (client) => {
      const row = (await client.query(
        ...
        [input.readingId, input.destinationCourtId ?? null, input.destinationCourtName,
          input.transmissionReference, input.transmittedAt, input.sevenDayCompliant,
          input.documentHash ?? null, input.transmittedBy, input.notes ?? null],
      )).rows[0];
  ...
  private mapReading(r: Record<string, unknown>): AppealDecisionReading {
    return {
      id: String(r.id), hearingId: String(r.hearing_id), version: Number(r.version),
      scheduledAt: String(r.scheduled_at), displayTimezone: String(r.display_timezone),
      deliveryMode: String(r.delivery_mode) as AppealDeliveryMode,
      determinationReference: String(r.determination_reference),
      virtualSessionReference: r.virtual_session_reference ? String(r.virtual_session_reference) : undefined,
      ...
      cassationDeadlineNote: r.cassation_deadline_note ? String(r.cassation_deadline_note) : undefined,
      ...
      updatedAt: String(r.updated_at), rowVersion: Number(r.row_version),
    };
  }

  private mapNoticeStep(r: Record<string, unknown>): AppealNoticeStep {
    return {
      id: String(r.id), readingId: String(r.reading_id),
      stepCode: String(r.step_code) as AppealNoticeStepCode,
      senderOrganizationId: String(r.sender_organization_id),
      recipientReference: String(r.recipient_reference), recipientName: String(r.recipient_name),
      channel: String(r.channel), officialReference: String(r.official_reference),
  ...
  private mapPresence(r: Record<string, unknown>): AppealPresenceRecord {
    return {
      ...
      partyReference: String(r.party_reference), partyName: String(r.party_name),
      ...
      verifiedBy: String(r.verified_by), verifiedAt: String(r.verified_at),
    };
  }
  ...
  private mapPublication(r: Record<string, unknown>): AppealPublication {
    return {
      id: String(r.id), readingId: String(r.reading_id),
      excerptReference: String(r.excerpt_reference), sourceSystemCode: String(r.source_system_code),
      documentHash: r.document_hash ? String(r.document_hash) : undefined,
      publishedAt: String(r.published_at), sameDayCompliant: Boolean(r.same_day_compliant),
      publishedBy: String(r.published_by), notes: r.notes ? String(r.notes) : undefined,
  ...
  private mapTransmission(r: Record<string, unknown>): AppealTransmission {
    return {
      ...
      transmittedAt: String(r.transmitted_at), sevenDayCompliant: Boolean(r.seven_day_compliant),
      documentHash: r.document_hash ? String(r.document_hash) : undefined,
      transmittedBy: String(r.transmitted_by), notes: r.notes ? String(r.notes) : undefined,
      createdAt: String(r.created_at),
    };
  }
}
