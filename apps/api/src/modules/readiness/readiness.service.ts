...
export class ReadinessService {
  ...
  async verifyIdentity(user: CurrentUser, hearingId: string, dto: IdentityVerificationDto, correlationId?: string) {
    ...
    if (['WITNESS', 'EXPERT', 'INTERPRETER'].includes(dto.participant_role ?? '')) {
      if (!dto.supervisor_officer_id || !dto.supervisor_officer_name) {
        throw new DomainError('SUPERVISOR_REQUIRED', 'Verifikasi saksi, ahli, atau penerjemah wajib mencantumkan petugas pengawas (SOP 10.9).', 422);
      }
    }
  ...
  async inspectRoom(user: CurrentUser, hearingId: string, dto: RoomInspectionDto, correlationId?: string) {
    requireRoles(user, ['CORRECTIONS', 'COURT_CLERK', 'IT_OPERATOR']);
    await this.core.getHearing(hearingId, user);
    const result: 'PASS' | 'FAIL' = dto.camera_full_view && dto.unauthorized_person_absent && dto.confidentiality_ready ? 'PASS' : 'FAIL';
  ...
  async submit(user: CurrentUser, hearingId: string, dto: SubmitReadinessDto, correlationId?: string) {
    requireRoles(user, ['COURT_CLERK', 'PROSECUTOR', 'CORRECTIONS', 'IT_OPERATOR']);
    await this.core.getHearing(hearingId, user);
    if (!(await this.core.activeSchedule(hearingId, user))) throw new DomainError('SCHEDULE_REQUIRED', 'An active schedule is required.', 409);
    const noticeGate = await this.notices.gate(hearingId, user);
    if (!noticeGate.ready) throw new DomainError('NOTICE_ACK_REQUIRED', 'Required official notices must be acknowledged before readiness submission.', 409, noticeGate);
    const organization = await this.core.getOrganization(user.organizationId, user);
    if (organization.type === 'CORRECTIONS') {
      const verification = await this.repository.latestVerificationStatus(hearingId, user.organizationId, user);
      if (!verification.identity || !verification.room) {
        throw new DomainError('VERIFICATION_REQUIRED', 'Corrections readiness requires passed identity verification and room inspection.', 409, {
          identity_verified: verification.identity,
          room_inspected: verification.room,
        });
      }
    }
    const itemReady = dto.items.filter((item) => item.required !== false).every((item) => item.result === 'PASS');
    const technical = dto.technical_test;
    const technicalReady = [technical.camera, technical.microphone, technical.audio, technical.primary_network, technical.provider_access].every((item) => item === 'PASS')
      && ['PASS', 'NA'].includes(technical.backup_network);
