...
import type {
  ImportJobRequestDto,
  ManualHearingDto,
  ReturnManualHearingDto,
  UpdateManualHearingDto
} from './dto.js';
...
export class HearingIntakeService {
  ...
  list(user: CurrentUser, status?: string, query?: string) {
    return this.repository.list(user, { status, query });
  }
  get(user: CurrentUser, hearingId: string) {
    return this.repository.get(hearingId, user);
  }
  referenceData(user: CurrentUser) {
    return this.repository.referenceData(user);
  }
  ...
  async create(user: CurrentUser, dto: ManualHearingDto, correlationId?: string) {
    ...
    await this.audit.append(
      {
        eventType: 'MANUAL_HEARING_INTAKE_CREATED',
        objectType: 'HEARING',
        objectId: item.id,
        actorUserId: user.id,
        ...
        payload: {
          ...
          hearing_sequence: item.hearingSequence,
  ...
  async update(
    ...
    await this.audit.append(
      {
        eventType: 'MANUAL_HEARING_INTAKE_UPDATED',
        objectType: 'HEARING',
        objectId: hearingId,
        actorUserId: user.id,
        actorOrganizationId: user.organizationId,
  ...
  async submit(user: CurrentUser, hearingId: string, correlationId?: string) {
    ...
    await this.audit.append(
      {
        eventType: 'MANUAL_HEARING_INTAKE_SUBMITTED',
        objectType: 'HEARING',
        objectId: hearingId,
        actorUserId: user.id,
        actorOrganizationId: user.organizationId,
        correlationId,
        payload: { intake_status: item.intakeStatus }
      },
      user
    );
    return item;
  ...
  async activate(user: CurrentUser, hearingId: string, correlationId?: string) {
    ...
    if (before.createdBy === user.id)
      throw new DomainError(
        ...
        'Pengguna yang membuat data tidak boleh mengaktifkan data yang sama.',
        409,
        { hearingId }
      );
    ...
    await this.audit.append(
      {
        eventType: 'MANUAL_HEARING_INTAKE_ACTIVATED',
        objectType: 'HEARING',
        objectId: hearingId,
        actorUserId: user.id,
        actorOrganizationId: user.organizationId,
  ...
  async returnForCorrection(
    ...
    await this.audit.append(
      {
        eventType: 'MANUAL_HEARING_INTAKE_RETURNED',
        objectType: 'HEARING',
        objectId: hearingId,
        actorUserId: user.id,
        actorOrganizationId: user.organizationId,
        correlationId,
        payload: { reason: dto.reason, intake_status: item.intakeStatus }
      },
      user
    );
    return item;
  ...
  async reopen(user: CurrentUser, hearingId: string, correlationId?: string) {
    ...
    await this.audit.append(
      {
        eventType: 'MANUAL_HEARING_INTAKE_REOPENED',
        objectType: 'HEARING',
        objectId: hearingId,
        actorUserId: user.id,
        actorOrganizationId: user.organizationId,
  ...
  async requestImport(user: CurrentUser, dto: ImportJobRequestDto): Promise<never> {
    requireRoles(user, ['COURT_CLERK']);
    return this.importGateway.requestPreview({
  ...
  private assertCourtScope(user: CurrentUser, courtOrganizationId: string): void {
    ...
    if (!user.organizationIds.includes(courtOrganizationId)) {
      throw new DomainError(
        'FORBIDDEN_COURT_SCOPE',
        'Panitera Pengganti hanya dapat membuat atau mengubah data persidangan pada pengadilan yang menjadi ruang lingkupnya.',
        403,
        { courtOrganizationId }
      );
    }
  ...
  private map(dto: ManualHearingDto): ManualHearingIntakeInput {
    return {
      ...
      caseTitle: dto.case_title ?? `Perkara ${dto.case_type_code} - ${dto.case_number}`,
      ...
      defendants: (dto.defendants || []).map((item) => ({
        ...
        appealRole: item.appeal_role
      })),
      prosecutors: (dto.prosecutors || []).map((item) => ({
        displayName: item.display_name,
        alias: item.alias,
        protectedIdentity: item.protected_identity,
        custodyStatus: item.custody_status,
