...
export class HearingIntakeController {
  ...
  referenceData(@CurrentUserContext() user: CurrentUser) {
    return this.service.referenceData(user);
  }
  ...
  async list(
    ...
    return { items: await this.service.list(user, status, query) };
  }
  ...
  @RequirePermissions('hearing.intake.read')
  get(@CurrentUserContext() user: CurrentUser, @Param('hearingId') hearingId: string) {
    return this.service.get(user, hearingId);
  }
  ...
  update(
    ...
    @Body() dto: UpdateManualHearingDto,
    @Headers('x-correlation-id') correlationId?: string
  ) {
    return this.service.update(user, hearingId, dto, correlationId);
  }
  ...
  submit(
    ...
    return this.service.submit(user, hearingId, correlationId);
  }
  ...
  activate(
    ...
    return this.service.activate(user, hearingId, correlationId);
  }
  ...
  returnForCorrection(
    @CurrentUserContext() user: CurrentUser,
    @Param('hearingId') hearingId: string,
    @Body() dto: ReturnManualHearingDto,
    @Headers('x-correlation-id') correlationId?: string
  ) {
    return this.service.returnForCorrection(user, hearingId, dto, correlationId);
  }
  ...
  reopen(
    ...
    return this.service.reopen(user, hearingId, correlationId);
  }

  @Get('hearing-import/sources')
  @RequirePermissions('hearing.import.read')
  sources(@CurrentUserContext() user: CurrentUser) {
    return this.service.importSources(user);
  }

  @Post('hearing-import/jobs')
  @RequirePermissions('hearing.import.execute')
  requestImport(@CurrentUserContext() user: CurrentUser, @Body() dto: ImportJobRequestDto) {
    return this.service.requestImport(user, dto);
  }
}
