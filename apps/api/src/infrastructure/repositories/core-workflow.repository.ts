...
export class CoreWorkflowRepository {
  ...
  async listHearings(user: CurrentUser): Promise<HearingRecord[]> {
    if (!this.mode.postgres) {
      ...
      const allowed = new Set(this.memory.hearingAssignments.filter((item) => user.organizationIds.includes(item.organizationId)).map((item) => item.hearingId));
      for (const assignment of this.memory.hearingUserAssignments.filter((item) => item.userId === user.id && item.active)) allowed.add(assignment.hearingId);
    ...
    return this.pg.transactionAs(user, async (client) => {
      ...
      return result.rows.map((row) => ({
        id: String(row.id), caseNumber: String(row.case_number), type: String(row.hearing_type), state: String(row.state),
        caseId: row.case_id ? String(row.case_id) : undefined, hearingSequence: row.hearing_sequence ? Number(row.hearing_sequence) : undefined,
        intakeStatus: row.intake_status ? String(row.intake_status) : undefined, dataSource: row.data_source ? String(row.data_source) : undefined,
  ...
  async hasActiveIntake(hearingId: string, user: CurrentUser, client?: PoolClient): Promise<boolean> {
    if (!this.mode.postgres) {
      const hearing = await this.getHearing(hearingId, user);
      return (hearing.intakeStatus ?? 'ACTIVE') === 'ACTIVE';
    }
    const query = async (connection: PoolClient) => {
      const result = await connection.query(`select intake_status from hearings where id=$1`, [hearingId]);
      return result.rows[0]?.intake_status === 'ACTIVE';
    };
    return client ? query(client) : this.pg.transactionAs(user, query);
  }

  async assertActiveIntake(hearingId: string, user: CurrentUser, client?: PoolClient): Promise<void> {
    if (!(await this.hasActiveIntake(hearingId, user, client))) {
      throw new DomainError('HEARING_DATA_NOT_ACTIVE', 'Data awal persidangan harus berstatus ACTIVE sebelum workflow berikutnya dijalankan.', 409, { hearingId });
    }
  }
  ...
  async updateHearingState(
    ...
    const update = async (connection: PoolClient) => {
      ...
      if (result.rowCount !== 1) {
        throw new DomainError('OPTIMISTIC_CONCURRENCY_CONFLICT', 'Hearing was changed by another transaction.', 409, {
          hearingId,
          expectedRowVersion,
        });
      }
  ...
  async getOrganization(id: string, user: CurrentUser): Promise<OrganizationRecord> {
    if (!this.mode.postgres) {
      ...
      if (!organization) throw new DomainError('ORGANIZATION_NOT_FOUND', 'User organization was not found.', 404);
      return organization;
    }
    const rows = await this.pg.transactionAs(user, async (client) =>
      (await client.query('select id,name,organization_type from organizations where id=$1 and active=true', [id])).rows,
    );
    const row = rows[0];
    if (!row) throw new DomainError('ORGANIZATION_NOT_FOUND', 'User organization was not found.', 404);
    return { id: String(row.id), name: String(row.name), type: String(row.organization_type) as OrganizationType };
  }

  async requiredOrganizationTypes(hearingId: string, user: CurrentUser): Promise<OrganizationType[]> {
    if (!this.mode.postgres) {
      return this.memory.hearingAssignments
        .filter((item) => item.hearingId === hearingId)
        .map((item) => this.memory.organizations.find((organization) => organization.id === item.organizationId)?.type)
  ...
  async createDetermination(
    input: { hearingId: string; decision: 'APPROVED' | 'REJECTED'; hearingMode?: 'LANGSUNG' | 'ELEKTRONIK' | 'HYBRID'; officialReference: string; reason: string },
    user: CurrentUser,
  ): Promise<DeterminationRecord> {
    if (!this.mode.postgres) {
      await this.getHearing(input.hearingId, user);
      const version = this.memory.determinations.filter((item) => item.hearingId === input.hearingId).length + 1;
    ...
    return this.pg.transactionAs(user, async (client) => {
      await this.getHearing(input.hearingId, user, client);
      await client.query('select pg_advisory_xact_lock(hashtextextended($1,0))', [`determination:${input.hearingId}`]);
      ...
      await client.query('update judicial_determinations set is_current=false where hearing_id=$1 and is_current=true', [input.hearingId]);
      const result = await client.query(
        ...
        [input.hearingId, version, input.decision, input.hearingMode ?? null, input.officialReference, input.reason, user.id],
      );
    ...
    if (!this.mode.postgres) {
      return this.memory.determinations
        ...
        .map((item, index) => ({
          ...
          isCurrent: (item as Partial<DeterminationRecord>).isCurrent ?? index === this.memory.determinations.length - 1,
  ...
  async hasApprovedDetermination(hearingId: string, user: CurrentUser): Promise<boolean> {
    if (!this.mode.postgres) {
      return this.memory.determinations.some((item) => item.hearingId === hearingId && item.decision === 'APPROVED');
    }
    const rows = await this.pg.transactionAs(user, async (client) =>
      (await client.query(
        `select exists(
  ...
  async getProposal(id: string, user: CurrentUser, client?: PoolClient): Promise<ProposalRecord> {
    if (!this.mode.postgres) {
      const item = this.memory.proposals.find((proposal) => proposal.id === id) as ProposalRecord | undefined;
  ...
  async activeSchedulesForConflict(proposal: ProposalRecord, user: CurrentUser): Promise<ScheduleRecord[]> {
    if (!this.mode.postgres) {
      return this.memory.schedules.map((schedule) => ({
        ...schedule,
        displayTimezone: (schedule as Partial<ScheduleRecord>).displayTimezone ?? proposal.displayTimezone,
    ...
    return this.pg.transactionAs(user, async (client) => {
      ...
      for (const row of result.rows) {
        ...
        schedules.push(this.mapSchedule(
          row,
          resourceRows.rows.map((resource) => ({
            resourceType: String(resource.resource_type) as ScheduleResource['resourceType'],
            resourceId: String(resource.resource_id),
  ...
  async saveConflictCheck(
    ...
    return this.pg.transactionAs(user, async (client) => {
      ...
      if (update.rowCount !== 1) {
        throw new DomainError('OPTIMISTIC_CONCURRENCY_CONFLICT', 'Schedule proposal was changed by another transaction.', 409, {
          proposalId,
      ...
      for (const conflict of conflicts) {
        await client.query(
          `insert into schedule_conflicts(proposal_id,conflict_code,severity,message,resource_type,resource_id)
           values($1,$2,$3,$4,$5,$6)`,
          [proposalId, conflict.code, conflict.severity, conflict.message, conflict.resourceType ?? null, conflict.resourceId ?? null],
        );
      }
  ...
  async conflicts(proposalId: string, user: CurrentUser, client?: PoolClient): Promise<ScheduleConflict[]> {
  ...
  async approveProposal(
    ...
    if (!this.mode.postgres) {
      ...
      if (proposal.status !== 'CHECKED') throw new DomainError('CONFLICT_CHECK_REQUIRED', 'Conflict check is required before approval.', 409);
      if (conflicts.some((item) => item.severity === 'REQUIRED')) throw new DomainError('CONFLICT_UNRESOLVED', 'Required scheduling conflicts remain.', 409, conflicts);
      for (const schedule of this.memory.schedules.filter((item) => item.hearingId === proposal.hearingId && item.status === 'ACTIVE')) schedule.status = 'SUPERSEDED';
      const schedule: ScheduleRecord = {
        ...
        version: this.memory.schedules.filter((item) => item.hearingId === proposal.hearingId).length + 1,
        status: 'ACTIVE',
    ...
    return this.pg.transactionAs(user, async (client) => {
      const proposal = await this.getProposal(proposalId, user, client);
      if (proposal.status !== 'CHECKED') throw new DomainError('CONFLICT_CHECK_REQUIRED', 'Conflict check is required before approval.', 409);
      const blocking = (await this.conflicts(proposalId, user, client)).filter((item) => item.severity === 'REQUIRED');
      if (blocking.length) throw new DomainError('CONFLICT_UNRESOLVED', 'Required scheduling conflicts remain.', 409, blocking);
      await client.query('select pg_advisory_xact_lock(hashtextextended($1,0))', [`schedule:${proposal.hearingId}`]);
      const expected = expectedRowVersion ?? proposal.rowVersion;
      ...
      const result = await client.query(
        ...
        [proposal.hearingId, version, proposal.startAt, proposal.endAt, proposal.displayTimezone, reason, user.id],
      );
  ...
  async schedules(hearingId: string, user: CurrentUser): Promise<ScheduleRecord[]> {
    if (!this.mode.postgres) {
      return this.memory.schedules.filter((item) => item.hearingId === hearingId).map((schedule) => ({
        ...schedule,
  ...
  async activeSchedule(hearingId: string, user: CurrentUser, client?: PoolClient): Promise<ScheduleRecord | undefined> {
    ...
    const query = async (connection: PoolClient) => {
      ...
      return this.mapSchedule(row, resources.rows.map((resource) => ({
        resourceType: String(resource.resource_type) as ScheduleResource['resourceType'],
        resourceId: String(resource.resource_id),
        requirement: String(resource.requirement) as ScheduleResource['requirement'],
      })));
  ...
  private mapDetermination(row: Record<string, unknown>): DeterminationRecord {
    return {
      ...
      hearingMode: row.hearing_mode ? String(row.hearing_mode) as DeterminationRecord['hearingMode'] : undefined,
  ...
  async saveAgendaItems(
    ...
    if (!this.mode.postgres) {
      this.memory.hearingAgendaItems = this.memory.hearingAgendaItems.filter((i) => i.hearingId !== hearingId);
    ...
    return this.pg.transactionAs(user, async (client) => {
      await client.query('select pg_advisory_xact_lock(hashtextextended($1,0))', [`agenda:${hearingId}`]);
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        const result = await client.query(
          ...
          [hearingId, i + 1, item.itemType, item.itemDescription, item.estimatedDurationMinutes ?? 30, user.id],
        );
        const row = result.rows[0];
        newItems.push({
          id: String(row.id),
