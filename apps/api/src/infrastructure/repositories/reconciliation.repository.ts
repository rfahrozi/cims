...
export class ReconciliationRepository {
  ...
  async request(
    ...
    if (!this.mode.postgres) {
      ...
      const run: ReconciliationRunRecord = {
        ...
        mismatchCount: 0,
        missingCount: 0,
        items: [{ id: randomUUID(), fieldPath: '$', cimsValue: 'MEMORY', sourceValue: 'MEMORY', result: 'MATCHED' }],
      };
  ...
  async list(hearingId: string, user: CurrentUser): Promise<ReconciliationRunRecord[]> {
    if (!this.mode.postgres) return [...this.memory.values()].filter((item) => item.hearingId === hearingId);
    return this.pg.transactionAs(user, async (client) => {
      const result = await client.query(
  ...
  async claimForProcessing(id: string): Promise<{ id: string; hearingId: string; sourceSystem: string } | null> {
    ...
    return this.pg.transaction(async (client) => {
      ...
      return row ? { id: String(row.id), hearingId: String(row.hearing_id), sourceSystem: String(row.source_system) } : null;
    });
  }
  ...
  async complete(id: string, cims: Record<string, unknown>, source: Record<string, unknown>): Promise<void> {
    ...
    await this.pg.transaction(async (client) => {
      ...
      for (const item of items) {
        await client.query(
          ...
          [id, item.fieldPath, JSON.stringify(item.cimsValue ?? null), JSON.stringify(item.sourceValue ?? null), item.result],
        );
      }
  ...
  private async getWithClient(id: string, client: import('pg').PoolClient): Promise<ReconciliationRunRecord> {
    const result = await client.query(
  ...
  private compare(cims: Record<string, unknown>, source: Record<string, unknown>): ReconciliationRunRecord['items'] {
