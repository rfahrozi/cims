...
export class VirtualSessionsRepository {
  ...
  async requestProvision(
    ...
    if (!this.mode.postgres) {
      const existing = [...this.memory.virtualSessions]
        .reverse()
        .find(
          (item) =>
            item.hearingId === input.hearingId && ['REQUESTED', 'READY'].includes(item.state)
        );
  ...
  async get(hearingId: string, user: CurrentUser): Promise<HydratedVirtualSession | null> {
    if (!this.mode.postgres) {
      const session = [...this.memory.virtualSessions]
        .reverse()
        .find((item) => item.hearingId === hearingId);
      return session
        ? {
            ...session,
            rowVersion: 1,
    ...
    if (!this.mode.postgres) {
      ...
      return session
        ? {
            ...session,
            rowVersion: 1,
            rooms: this.memory.virtualRooms.filter((item) => item.virtualSessionId === id)
          }
        : null;
    }
    return this.pg.transaction((client) =>
      this.hydrateWithClient(id, client).catch((error) => {
  ...
  async markReady(
    ...
    if (!this.mode.postgres) {
      const session = this.memory.virtualSessions.find((item) => item.id === id);
      if (!session)
        throw new DomainError('VIRTUAL_SESSION_NOT_FOUND', 'Virtual session was not found.', 404);
      session.providerSessionReference = providerSessionReference;
      session.state = 'READY';
      session.updatedAt = new Date().toISOString();
      for (const room of rooms)
        this.memory.virtualRooms.push({ id: this.memory.id(), virtualSessionId: id, ...room });
      return;
    }
    await this.pg.transaction(async (client) => {
      ...
      if (updated.rowCount !== 1) {
        throw new DomainError(
          'VIRTUAL_SESSION_STATE_INVALID',
          'Virtual session is not in REQUESTED state.',
          409,
          { id }
        );
      }
  ...
  async isReady(hearingId: string, user: CurrentUser): Promise<boolean> {
    if (!this.mode.postgres)
      return this.memory.virtualSessions.some(
