...
export class HearingAccessService {
  ...
  async canAccess(user: CurrentUser, hearingId: string): Promise<boolean> {
    if (user.roles.includes('SYSTEM_ADMIN') || user.hearingAssignments.includes(hearingId))
      return true;
    if (!this.mode.postgres) {
      if (
        this.memory.hearingUserAssignments.some(
          (item) => item.hearingId === hearingId && item.userId === user.id && item.active
        )
      )
        return true;
      ...
      return assignedOrganizations.some((organizationId) =>
        user.organizationIds.includes(organizationId)
      );
    }
    const result = await this.pg.transaction(async (client) =>
      client.query(
        `select exists(
