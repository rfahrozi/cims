...
export class FieldCryptoService {
  ...
  constructor(config: ConfigService) {
    const environment =
      (config && config.get ? config.get<string>('NODE_ENV') : undefined) ?? 'development';

    const loaded: KeyEntry[] = [];

    // ── Load semua versi kunci ─────────────────────────────────────────────
    // V1 — kunci asli (FIELD_ENCRYPTION_KEY)
    const raw1 = secretValue(config, 'FIELD_ENCRYPTION_KEY') ?? '';
    ...
    const raw2 = secretValue(config, 'FIELD_ENCRYPTION_KEY_V2') ?? '';
    if (raw2) {
    ...
    const raw3 = secretValue(config, 'FIELD_ENCRYPTION_KEY_V3') ?? '';
    if (raw3) {
