...
export class IdempotencyService {
  ...
  requestHash(body: unknown): string { return createHash('sha256').update(JSON.stringify(body ?? null)).digest('hex'); }
  async reserve(scope: string, key: string, body: unknown, ttlSeconds = 86_400): Promise<void> {
    ...
    const rows = await this.pg.query<{ request_hash: string }>(
      `insert into api_idempotency_keys(scope, idempotency_key, request_hash, expires_at)
       ...
       returning request_hash`, [scope, key, hash, ttlSeconds]);
    if (rows[0]?.request_hash !== hash) throw new DomainError('IDEMPOTENCY_KEY_REUSED', 'Idempotency key was reused with a different request.', 409);
  }
}
