...
export class PolicyGuard implements CanActivate {
  constructor(private readonly reflector: Reflector, private readonly hearingAccess: HearingAccessService) {}
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const request = context.switchToHttp().getRequest<{ user?: CurrentUser; params?: Record<string, string> }>();
    ...
    if (!user.roles.includes('SYSTEM_ADMIN') && !user.permissions.includes('*')) {
      if (!permissions.every((permission) => user.permissions.includes(permission))) {
        throw new DomainError('FORBIDDEN', 'Permission policy rejected the operation.', 403, { required: permissions });
      }
      const hearingId = request.params?.hearingId ?? request.params?.id;
      if (hearingId && hearingId.startsWith('hearing') && !(await this.hearingAccess.canAccess(user, hearingId))) {
        throw new DomainError('FORBIDDEN', 'Hearing assignment and organization policy rejected the operation.', 403, { hearingId });
      }
    }
    return true;
  }
