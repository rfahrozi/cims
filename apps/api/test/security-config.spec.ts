...
describe('Runtime Security Config', () => {
  ...
  it('forbids DEV auth in production', () => {
    ...
    const env = validateEnvOrThrow(raw);
    expect(() => enforceRuntimeSecurityPolicy(env, () => undefined)).toThrow(/forbidden in production/);
  });
  ...
  it('requires POSTGRES to have a DATABASE_URL', () => {
    ...
    const env = validateEnvOrThrow(raw);
    expect(() => enforceRuntimeSecurityPolicy(env, () => undefined)).toThrow(/DATABASE_URL is required/);
  });
});