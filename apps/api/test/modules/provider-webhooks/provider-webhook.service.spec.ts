...
describe('ProviderWebhookService', () => {
  ...
  it('rejects missing configuration', async () => {
    ...
    await expect(service.ingest('ZOOM', Buffer.from('{}'), {}, {})).rejects.toThrow(
  ...
  it('rejects expired timestamp (anti-replay window)', async () => {
    ...
    await expect(
      service.ingest('ZOOM', rawBody, {}, { timestamp: oldTimestamp, signature })
  ...
  it('rejects future timestamp (anti-replay window)', async () => {
    ...
    await expect(
      service.ingest('ZOOM', rawBody, {}, { timestamp: futureTimestamp, signature })
    ).rejects.toThrow('Webhook timestamp is outside the accepted tolerance.');
  });
  ...
  it('rejects invalid signature', async () => {
    ...
    await expect(service.ingest('ZOOM', rawBody, {}, { timestamp, signature })).rejects.toThrow(
