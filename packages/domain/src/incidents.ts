...
export function incidentNotificationDeadline(type: IncidentType, occurredAt: string): string | undefined {
  const base = Date.parse(occurredAt);
  if (!Number.isFinite(base)) throw new DomainError('INVALID_INCIDENT_TIME', 'Incident time is invalid.', 422);
...
export function incidentBlocksHearing(type: IncidentType, severity: IncidentSeverity): boolean {
  ...
  if (severity === 'HIGH') return type === 'TECHNICAL' || type === 'CYBER' || type === 'FORCE_MAJEURE';
  return false;
}

export function transitionIncident(current: IncidentStatus, action: IncidentAction): IncidentStatus {
  ...
  if (!next) throw new DomainError('INVALID_INCIDENT_TRANSITION', `Incident cannot perform ${action} from ${current}.`, 409, { current, action });
  return next;
}
