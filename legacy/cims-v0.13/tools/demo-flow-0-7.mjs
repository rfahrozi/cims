...
const port = 43207,
  provider = spawn(process.execPath, ['services/mock-video-provider/src/server.mjs'], {
    cwd: process.cwd(),
    env: {
      ...process.env,
...
try {
  ...
  const schedule = await prepareActiveSchedule(t.baseUrl, tokens);
  const notices = await prepareNotices(t.baseUrl, tokens);
  await prepareReadiness(t.baseUrl, tokens);
  const r = await fetch(
    `${t.baseUrl}/api/v1/hearings/${DEMO.hearingId}/virtual-session:provision`,
    {
      method: 'POST',
      headers: authHeaders(tokens.clerk, `demo-${crypto.randomUUID()}`),
      body: JSON.stringify({ recording_policy: 'COURT_CONTROLLED' })
    }
  );
  const session = await r.json();
  const g = await fetch(`${t.baseUrl}/api/v1/hearings/${DEMO.hearingId}/gate-status`, {
