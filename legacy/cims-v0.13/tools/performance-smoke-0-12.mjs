...
const total = Number(process.env.PERF_REQUESTS ?? 400);
...
try {
    ...
    while (true) {
      ...
      try {
        ...
        const response = await fetch(app.baseUrl + path, {
          headers: path === '/health' ? {} : authHeaders(token)
        });
  ...
  const result = {
    ...
    target: { success_rate_min: 0.99, p95_ms_max: 250 },
    passed: success / total >= 0.99 && pct(0.95) <= 250,
