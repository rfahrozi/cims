...
export function loadZoomAdapterConfig(overrides = {}) {
  return {
    ...
    dbPath:
      overrides.dbPath ??
      process.env.ZOOM_ADAPTER_DB_PATH ??
      path.join(root, 'var/zoom-adapter.sqlite'),
    ...
    apiBaseUrl: String(
      overrides.apiBaseUrl ?? process.env.ZOOM_API_BASE_URL ?? 'https://api.zoom.us/v2'
    ).replace(/\/$/, ''),
    webhookSecretToken: overrides.webhookSecretToken ?? process.env.ZOOM_WEBHOOK_SECRET_TOKEN ?? '',
    cimsWebhookUrl:
      overrides.cimsWebhookUrl ??
      process.env.CIMS_WEBHOOK_URL ??
      'http://127.0.0.1:4000/api/v1/provider-webhooks/video',
    cimsProviderWebhookSecret:
      overrides.cimsProviderWebhookSecret ?? process.env.CIMS_PROVIDER_WEBHOOK_SECRET ?? '',
    ...
    requestTimeoutMs: Number(
      overrides.requestTimeoutMs ?? process.env.ZOOM_REQUEST_TIMEOUT_MS ?? 12000
    ),
    webhookToleranceSeconds: Number(
      overrides.webhookToleranceSeconds ?? process.env.ZOOM_WEBHOOK_TOLERANCE_SECONDS ?? 300
    ),
    allowManualLiveControls:
      overrides.allowManualLiveControls ?? bool(process.env.ZOOM_ALLOW_MANUAL_LIVE_CONTROLS, true),
    registrationEnabled:
      overrides.registrationEnabled ?? bool(process.env.ZOOM_REGISTRATION_ENABLED, true)
  };
}
