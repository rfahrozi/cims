...
export function validationResponse(secretToken, body) {
  ...
  assert(body?.event==='endpoint.url_validation' && plainToken,'WEBHOOK_VALIDATION_INVALID','Zoom URL validation payload is invalid.',400);
  return {plainToken,encryptedToken:createHmac('sha256',secretToken).update(plainToken).digest('hex')};
}
...
export function verifyZoomWebhook(config,headers,rawBody) {
  ...
  assert(timestamp&&signature,'WEBHOOK_SIGNATURE_REQUIRED','Zoom webhook signature headers are required.',401);
  if (Math.abs(Date.now()-Number(timestamp)*1000)>config.webhookToleranceSeconds*1000) throw new AdapterError('WEBHOOK_EXPIRED','Zoom webhook timestamp is outside the accepted window.',401);
  const message=`v0:${timestamp}:${rawBody}`;
  const expected=`v0=${createHmac('sha256',config.webhookSecretToken).update(message).digest('hex')}`;
  if (!safeEqual(signature,expected)) throw new AdapterError('WEBHOOK_SIGNATURE_INVALID','Zoom webhook signature is invalid.',401);
  return true;
}
...
export function translateZoomEvent(body) {
  ...
  return {
    ...
    provider_participant_reference:participant?.registrant_id||participant?.participant_user_id||participant?.id||null,
...
export async function emitCimsWebhook(config,event,fetchImpl=fetch) {
  if (!config.cimsWebhookUrl || !config.cimsProviderWebhookSecret) return {delivered:false,reason:'CIMS_WEBHOOK_NOT_CONFIGURED'};
  const timestamp=new Date().toISOString();
  const raw=JSON.stringify(event);
  const signature=createHmac('sha256',config.cimsProviderWebhookSecret).update(`${timestamp}.${raw}`).digest('hex');
  const response=await fetchImpl(config.cimsWebhookUrl,{method:'POST',headers:{'content-type':'application/json','X-Provider-Signature':signature,'X-Provider-Timestamp':timestamp,'X-Provider-Event-Id':event.event_id},body:raw});
  if (!response.ok) throw new AdapterError('CIMS_WEBHOOK_DELIVERY_FAILED',`CIMS webhook returned HTTP ${response.status}.`,503,{status:response.status},true);
