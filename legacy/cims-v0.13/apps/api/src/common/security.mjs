...
export function verifyOtp(code, secret, config, at = Date.now()) {
  if (config.otpMode === 'fixed')
    return timingSafeEqual(Buffer.from(String(code)), Buffer.from(String(config.fixedOtp)));
  for (const drift of [-1, 0, 1]) {
    const expected = generateTotp(secret, at + drift * 30_000);
    if (
      String(code).length === expected.length &&
      timingSafeEqual(Buffer.from(String(code)), Buffer.from(expected))
    )
      return true;
  }
  return false;
...
export function verifyAccessToken(token, secret) {
  ...
  if (parts.length !== 3)
    throw new DomainError('UNAUTHENTICATED', 'Bearer token is missing or invalid.', 401);
  ...
  try {
    ...
    throw new DomainError('UNAUTHENTICATED', 'Bearer token payload is invalid.', 401);
  }
  if (!payload.exp || payload.exp <= Math.floor(Date.now() / 1000))
    throw new DomainError('TOKEN_EXPIRED', 'Bearer token has expired.', 401);
  return payload;
}
...
export function bearerToken(request) {
  ...
  if (!authorization?.startsWith('Bearer '))
    throw new DomainError('UNAUTHENTICATED', 'Bearer token is required.', 401);
