...
export class FixedWindowRateLimiter {
  ...
  consume(key, at = Date.now()) {
    ...
    if (existing.count > this.max) {
      ...
      throw new DomainError('RATE_LIMITED', 'Too many requests. Retry after the configured window.', 429, {retry_after_seconds: retryAfterSeconds});
    }
    return {remaining: this.max - existing.count, resetAt: existing.resetAt};
  }

  prune(at = Date.now()) {
    for (const [key, value] of this.buckets.entries()) if (at >= value.resetAt) this.buckets.delete(key);
  }
}
