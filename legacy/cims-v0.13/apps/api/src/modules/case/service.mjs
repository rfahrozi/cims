...
export class CaseService {
  ...
  listHearings(context) {
    ...
    if (context.isSystemAdmin)
      return this.db.all(
        `select h.*, c.case_number, c.case_type from hearings h join case_references c on c.id=h.case_reference_id order by h.created_at desc`
      );
    return this.db.all(
      `select h.*, c.case_number, c.case_type from hearings h
  ...
  getHearing(hearingId) {
    const hearing = this.db.get(
      `select h.*, c.case_number, c.case_type from hearings h
      join case_references c on c.id=h.case_reference_id where h.id=?`,
      hearingId
    );
