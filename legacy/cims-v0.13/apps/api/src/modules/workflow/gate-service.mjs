...
export class WorkflowGateService {
  constructor(db, determination, notice, readiness, virtual) {
    this.db = db;
  ...
  status(context, hearingId) {
    ...
    const determinationValid = Boolean(
      determination &&
    ...
    const notice = this.notice.gate(hearingId),
      readiness = this.readiness.gate(hearingId),
      virtual = this.virtual.gate(hearingId);
    const runtime = this.db.get('select * from hearing_runtime where hearing_id=?', hearingId);
    const participantCounts = this.db.get(
      ...
      from hearing_participants p left join participant_sessions ps on ps.participant_id=p.id where p.hearing_id=? and p.status='REGISTERED'`,
      hearingId
    );
    const incidentCounts = this.db.get(
      `select count(*) as open_count,sum(case when severity in ('HIGH','CRITICAL') then 1 else 0 end) as blocking_count from incidents where hearing_id=? and status in ('OPEN','MITIGATING')`,
      hearingId
    );
    ...
    return {
      ...
      determination_valid: determinationValid,
      ...
      readiness,
      virtual,
      runtime: runtime ?? { state: virtual.ready ? 'READY' : 'NOT_READY' },
      participants: {
        registered: Number(participantCounts.registered ?? 0),
        waiting: Number(participantCounts.waiting ?? 0),
        admitted: Number(participantCounts.admitted ?? 0)
      },
      incidents: {
        open: Number(incidentCounts.open_count ?? 0),
        blocking: Number(incidentCounts.blocking_count ?? 0)
      },
      next_gate: next
    };
  }
