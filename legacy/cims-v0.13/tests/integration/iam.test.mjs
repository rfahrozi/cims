...
test('IAM login, OTP, profile and admin authorization', async () => {
  ...
  try {
    ...
    response = await fetch(`${env.baseUrl}/api/v1/admin/users`, {headers: authHeaders(clerkToken)});
    assert.equal(response.status, 403);

    const adminToken = await login(env.baseUrl, 'admin@cims.local', 'Admin123!');
    response = await fetch(`${env.baseUrl}/api/v1/admin/users`, {headers: authHeaders(adminToken)});
    assert.equal(response.status, 200);
...
test('invalid OTP is rejected', async () => {
  const env = await startTestApp();
  try {
    let response = await fetch(`${env.baseUrl}/api/v1/auth/login`, {method: 'POST', headers: {'content-type': 'application/json'}, body: JSON.stringify({email: 'judge@cims.local', password: 'Judge123!'})});
    const challenge = await response.json();
    response = await fetch(`${env.baseUrl}/api/v1/auth/verify-otp`, {method: 'POST', headers: {'content-type': 'application/json'}, body: JSON.stringify({challenge_id: challenge.challenge_id, otp: '000000'})});
