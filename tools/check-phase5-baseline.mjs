...
for (const marker of ['SUBSTITUTE_CLERK', 'MAKER_CHECKER_REQUIRED', 'assertCourtScope']) {
  if (!service.includes(marker))
    throw new Error(`Hearing intake service marker missing: ${marker}`);
}
...
for (const marker of [
  ...
  if (!repository.includes(marker))
    throw new Error(`Hearing intake repository marker missing: ${marker}`);
}
...
if (!gates.includes("return 'HEARING_DATA'"))
...
const forbidden = [
  ...
  /HEARING_IMPORT_DATABASE_URL\s*=\s*\S+/i
];
const files = [];
function walk(dir) {
...
for (const root of ['apps', 'packages', 'services', 'database', 'docs', 'tools'])
...
console.log(
  `PASS Phase 5 baseline: ${required.length} required files and ${files.length} files scanned`
);
