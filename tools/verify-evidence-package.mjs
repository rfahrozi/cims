...
for (const item of value.manifest?.items ?? []) {
  ...
  if (actual !== item.contentHash)
    failures.push({ category: item.category, expected: item.contentHash, actual });
}
const manifestHash = hash(value.manifest);
if (manifestHash !== value.manifest_hash)
  failures.push({ category: 'MANIFEST', expected: value.manifest_hash, actual: manifestHash });
if (failures.length) {
...
console.log(
  JSON.stringify(
    { status: 'PASS', manifest_hash: manifestHash, item_count: value.manifest.items.length },
    null,
    2
  )
);
