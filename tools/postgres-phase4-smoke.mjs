...
const client = new pg.Client({ connectionString: process.env.DATABASE_URL, application_name: 'cims-phase4-smoke' });
...
try {
  const requiredTables = [
    'hearings', 'judicial_determinations', 'schedule_proposals', 'hearing_schedules',
    'official_notices', 'readiness_submissions', 'virtual_sessions', 'hearing_runtime',
    'audit_events', 'outbox_events', 'provider_webhook_events', 'video_provider_operations', 'reconciliation_runs',
  ];
  ...
  if (tableRows.rowCount !== requiredTables.length) {
    ...
    throw new Error(`Missing tables: ${requiredTables.filter((name) => !found.has(name)).join(', ')}`);
  }

  const ledger = await client.query('select count(*)::int as count from schema_migrations');
  if (Number(ledger.rows[0].count) < 3) throw new Error('Migration ledger does not contain all baseline migrations.');





  await client.query(`insert into court_cases(id,case_number,normalized_case_number,court_organization_id,prosecution_organization_id,created_by,updated_by,case_classification,case_type_code,case_title,data_source) values($1,$2,$2,$5,$5,'test','test','GENERAL_CRIMINAL','test','test','MANUAL'),($3,$4,$4,$6,$6,'test','test','GENERAL_CRIMINAL','test','test','MANUAL') on conflict do nothing`,[hearingId,`SMOKE/${suffix}`,`other-hearing-${suffix}`,`OTHER/${suffix}`,courtId,otherId]);
  ...
  await client.query(`insert into court_cases(id,case_number,normalized_case_number,court_organization_id,prosecution_organization_id,created_by,updated_by,case_classification,case_type_code,case_title,data_source) values($1,$2,$2,$5,$5,'test','test','GENERAL_CRIMINAL','test','test','MANUAL'),($3,$4,$4,$6,$6,'test','test','GENERAL_CRIMINAL','test','test','MANUAL') on conflict do nothing`,[hearingId,`SMOKE/${suffix}`,`other-hearing-${suffix}`,`OTHER/${suffix}`,courtId,otherId]);
  await client.query('insert into hearing_assignments(hearing_id,organization_id) values($1,$2)', [hearingId, courtId]);
  ...
  if (!uniqueBlocked) throw new Error('Single active schedule constraint did not block a duplicate.');
  ...
  try { await client.query('update audit_events set event_type=\'MUTATED\' where id=$1', [audit.rows[0].id]); }
  catch { immutable = true; await client.query('rollback to savepoint audit_immutable_test'); }
  ...
  await client.query(`insert into court_cases(id,case_number,normalized_case_number,court_organization_id,prosecution_organization_id,created_by,updated_by,case_classification,case_type_code,case_title,data_source) values($1,$2,$2,$5,$5,'test','test','GENERAL_CRIMINAL','test','test','MANUAL'),($3,$4,$4,$6,$6,'test','test','GENERAL_CRIMINAL','test','test','MANUAL') on conflict do nothing`,[hearingId,`SMOKE/${suffix}`,`other-hearing-${suffix}`,`OTHER/${suffix}`,courtId,otherId]);
  await client.query(`do $$ begin if not exists(select 1 from pg_roles where rolname='cims_smoke_app') then create role cims_smoke_app nologin; end if; end $$`);
  ...
  await client.query(`insert into court_cases(id,case_number,normalized_case_number,court_organization_id,prosecution_organization_id,created_by,updated_by,case_classification,case_type_code,case_title,data_source) values($1,$2,$2,$5,$5,'test','test','GENERAL_CRIMINAL','test','test','MANUAL'),($3,$4,$4,$6,$6,'test','test','GENERAL_CRIMINAL','test','test','MANUAL') on conflict do nothing`,[hearingId,`SMOKE/${suffix}`,`other-hearing-${suffix}`,`OTHER/${suffix}`,courtId,otherId]);
  await client.query(`select set_config('cims.is_system_admin','false',true), set_config('cims.hearing_assignments',$1,true), set_config('cims.organization_ids',$2,true)`, [hearingId, courtId]);
  const visible = await client.query('select id from hearings where id in ($1,$2)', [hearingId, `other-hearing-${suffix}`]);
  ...
  if (visible.rowCount !== 1 || visible.rows[0].id !== hearingId) throw new Error('RLS hearing scope failed.');

  const policies = await client.query(`select count(*)::int as count from pg_policies where schemaname='public'`);
  if (Number(policies.rows[0].count) < 15) throw new Error('Expected RLS policies were not installed.');

  const rlsTables = ['hearings','official_notices','participant_access_tokens','participant_sessions','attendance_events','consultation_sessions','incidents','incident_actions'];
  ...
  await client.query(`insert into court_cases(id,case_number,normalized_case_number,court_organization_id,prosecution_organization_id,created_by,updated_by,case_classification,case_type_code,case_title,data_source) values($1,$2,$2,$5,$5,'test','test','GENERAL_CRIMINAL','test','test','MANUAL'),($3,$4,$4,$6,$6,'test','test','GENERAL_CRIMINAL','test','test','MANUAL') on conflict do nothing`,[hearingId,`SMOKE/${suffix}`,`other-hearing-${suffix}`,`OTHER/${suffix}`,courtId,otherId]);
  await client.query('delete from hearings where id=any($1::text[])', [[hearingId, `other-hearing-${suffix}`]]);
