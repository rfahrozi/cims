...
if (missing.length) {
  console.error('Missing Phase 6 files:', missing);
  process.exit(1);
}
...
for (const marker of [
  ...
  'cims_block_mutation',
  'cims_guard_legal_hold_transition',
  'cims_guard_access_review_item_transition',
  'cims_guard_evidence_export_transition'
]) {
...
for (const marker of [
  ...
  if (!repository.includes(marker))
    throw new Error(`Governance repository marker missing: ${marker}`);
}
...
for (const root of ['apps', 'packages', 'services', 'database', 'docs', 'tools', 'infra'])
  if (existsSync(root)) walk(root);
const forbidden = [
  /BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY/,
  /EVIDENCE_STORAGE_API_KEY\s*=\s*[^\s#]+/i
];
for (const file of files) {
  if (file.endsWith('.env.example') || file.includes('check-')) continue;
...
for (const configFile of [
  ...
  if (
    ...
    /RETENTION_EXECUTION_ENABLED=true/i.test(text)
  )
    throw new Error(`Retention execution must remain disabled in ${configFile}`);
}
if (!existsSync('package-lock.json'))
  console.warn(
    'WARN package-lock.json is absent. Production readiness remains NO_GO until the official CI generates and reviews it.'
  );
console.log(
  `PASS Phase 6 baseline: ${required.length} required files and ${files.length} files scanned`
);
